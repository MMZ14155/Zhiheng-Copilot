...
vi.mock('../api', async (original) => {
  ...
  return {
    ...
    adminApi: {
      ...
      getLlmConfig: vi.fn(), updateLlmConfig: vi.fn(), testLlmConfig: vi.fn(),
    },
...
const llmConfig = { provider: 'kimi', baseUrl: 'https://api.moonshot.cn/v1', model: 'kimi-k2.5', timeoutSeconds: 60, inputPricePerMtok: '4', outputPricePerMtok: '16', apiKeySet: true, apiKeyMasked: '****5678', source: 'db' as const }
...
describe('管理页与项目创建', () => {
  beforeEach(() => {
    ...
    vi.mocked(adminApi.testLlmConfig).mockResolvedValue({ ok: true, detail: 'LLM 配置连接成功' })
  ...
  it('AI 配置仅回显掩码且留空保存不提交 apiKey', async () => {
    ...
    expect(keyInput.type).toBe('password'); expect(keyInput.value).toBe(''); expect(keyInput.placeholder).toBe('****5678')
    expect(screen.getByText(/当前 Key \*\*\*\*5678/)).toBeTruthy()
    await userEvent.click(screen.getByRole('button', { name: '保存配置' }))
    await waitFor(() => expect(adminApi.updateLlmConfig).toHaveBeenCalled())
    expect(adminApi.updateLlmConfig).toHaveBeenCalledWith(expect.not.objectContaining({ apiKey: expect.anything() }))
  })

  it('AI 配置连接测试展示服务端结果', async () => {
    render(<MemoryRouter><Admin /></MemoryRouter>); await screen.findByLabelText('API Key')
