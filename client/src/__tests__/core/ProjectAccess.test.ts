...
describe("canViewFile", () => {
  ...
  it("文件可见性完全继承项目可见性", () => {
    ...
    expect(canViewFile(manager, wsFile, baseProject)).toBe(
      canViewProject(manager, baseProject),
    );
    expect(canViewFile(outsider, trFile, baseProject)).toBe(
      canViewProject(outsider, baseProject),
    );
  });
});
