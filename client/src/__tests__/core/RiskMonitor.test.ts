...
import type { MonitorRiskConfig, MonitorThresholds } from '../../core/RiskMonitor'
...
function makeThresholds(overrides: Partial<MonitorThresholds> = {}): MonitorThresholds {
...
describe('evaluateProject - 规则1 时间进度（强制检测）', () => {
  ...
  it('进度使用率远低于阈值不产生超期风险', () => {
    ...
    const risks = evaluateProject(project, makeWorkspace({ deliverables: [] }), makeConfig())
    const overrun = risks.find((r) => r.type === 'schedule-overrun')
    expect(overrun).toBeUndefined()
  })
...
describe('evaluateProject - 规则1b 剩余工期不足90天', () => {
  ...
  it('剩余工期不足时与超期 block 同时存在', () => {
    const project = makeProject({ usedDays: 110, plannedDays: 100 })
    const risks = evaluateProject(project, makeWorkspace(), makeConfig())
    expect(risks.some((r) => r.type === 'schedule-overrun' && r.level === 'block')).toBe(true)
    expect(risks.some((r) => r.type === 'schedule-remaining' && r.level === 'warn')).toBe(true)
  })
...
describe('evaluateProject - 规则2 成本超支', () => {
  ...
  it('自定义阈值可调整触发边界', () => {
    ...
    const config = makeConfig({ thresholds: makeThresholds({ costWarn: 0.5, costBlock: 0.7 }) })
    const risks = evaluateProject(project, makeWorkspace(), config)
    const cost = risks.find((r) => r.type === 'cost-overrun')
    expect(cost).toBeDefined()
    expect(cost?.level).toBe('block')
...
describe('evaluateProject - 规则3 资料缺失', () => {
  it('必须交付物缺失产生 block', () => {
    ...
    const file = makeTrackedFile({ name: '合同文件', category: '合同', required: true, status: 'missing' })
    const risks = evaluateProject(project, makeWorkspace({ deliverables: [file] }), makeConfig())
  ...
  it('多个必须交付物缺失产生多条 block', () => {
    const project = makeProject({ usedDays: 5, plannedDays: 200 })
    const f1 = makeTrackedFile({ id: 'F1', name: '文件1', required: true, status: 'missing' })
    const f2 = makeTrackedFile({ id: 'F2', name: '文件2', required: true, status: 'missing' })
    const risks = evaluateProject(project, makeWorkspace({ deliverables: [f1, f2] }), makeConfig())
  ...
  it('enabledRules.documentMissing=false 时跳过资料缺失检测', () => {
    ...
    const risks = evaluateProject(project, makeWorkspace({ deliverables: [file] }), config)
    const missing = risks.find((r) => r.type === 'document-missing')
    expect(missing).toBeUndefined()
  })
...
describe('evaluateProject - 规则4 版本冲突', () => {
  it('2 个未冻结版本产生 warn', () => {
    ...
    const file = makeTrackedFile({
      versions: [makeVersion({ isFrozen: false }), makeVersion({ version: 'v2.0', isFrozen: false })],
    })
    const risks = evaluateProject(project, makeWorkspace({ deliverables: [file] }), makeConfig())
  ...
  it('1 个未冻结版本不产生版本冲突风险', () => {
    const project = makeProject({ usedDays: 5, plannedDays: 200 })
    const file = makeTrackedFile({ versions: [makeVersion({ isFrozen: false })] })
    const risks = evaluateProject(project, makeWorkspace({ deliverables: [file] }), makeConfig())
    const conflict = risks.find((r) => r.type === 'version-conflict')
  ...
  it('2 个已冻结版本不产生版本冲突风险', () => {
    const project = makeProject({ usedDays: 5, plannedDays: 200 })
    const file = makeTrackedFile({
      versions: [makeVersion({ isFrozen: true }), makeVersion({ version: 'v2.0', isFrozen: true })],
    })
    const risks = evaluateProject(project, makeWorkspace({ deliverables: [file] }), makeConfig())
    const conflict = risks.find((r) => r.type === 'version-conflict')
    expect(conflict).toBeUndefined()
...
describe('evaluateProject - 规则5 规则冲突（验收阶段）', () => {
  it('验收阶段缺失验收材料产生 block', () => {
    const project = makeProject({ stage: 'accepting' as ProjectStage, usedDays: 5, plannedDays: 200 })
  ...
  it('非验收阶段不产生规则冲突', () => {
    const project = makeProject({ stage: 'executing' as ProjectStage, usedDays: 5, plannedDays: 200 })
    const file = makeTrackedFile({ category: '交付成果' })
    const risks = evaluateProject(project, makeWorkspace({ deliverables: [file] }), makeConfig())
    const rule = risks.find((r) => r.type === 'rule-conflict')
    expect(rule).toBeUndefined()
  ...
  it('enabledRules.ruleConflict=false 时跳过规则冲突检测', () => {
    ...
    const risks = evaluateProject(project, makeWorkspace({ deliverables: [] }), config)
    const rule = risks.find((r) => r.type === 'rule-conflict')
    expect(rule).toBeUndefined()
  })
...
describe('evaluateProject - 综合场景', () => {
  it('健康项目仅剩余工期不足产生 warn', () => {
    const project = makeProject({ usedDays: 50, plannedDays: 100, cost: 50, budget: 100 })
    const risks = evaluateProject(project, makeWorkspace({ deliverables: [] }), makeConfig())
  ...
  it('超期+超支+缺失+规则冲突同时触发 block', () => {
    ...
    const file = makeTrackedFile({ name: '合同', category: '合同', required: true, status: 'missing' })
    const risks = evaluateProject(project, makeWorkspace({ deliverables: [file] }), makeConfig())
    expect(risks.some((r) => r.type === 'schedule-overrun' && r.level === 'block')).toBe(true)
    expect(risks.some((r) => r.type === 'schedule-remaining' && r.level === 'warn')).toBe(true)
    expect(risks.some((r) => r.type === 'cost-overrun' && r.level === 'block')).toBe(true)
    expect(risks.some((r) => r.type === 'document-missing' && r.level === 'block')).toBe(true)
    expect(risks.some((r) => r.type === 'rule-conflict' && r.level === 'block')).toBe(true)
  })
  ...
  it('无任何风险时返回空数组', () => {
    const project = makeProject({ usedDays: 5, plannedDays: 200, cost: 50, budget: 100 })
    const risks = evaluateProject(project, makeWorkspace({ deliverables: [] }), makeConfig())
...
describe('aggregateRisk', () => {
  ...
  it('仅 block 风险返回 level 为 block', () => {
    const risks: Risk[] = [
      { type: 'cost-overrun', level: 'block', reason: 'r', recommendation: 'a' },
    ]
    const result = aggregateRisk(risks)
  ...
  it('block 优先级高于 warn', () => {
    const risks: Risk[] = [
      { type: 'cost-overrun', level: 'warn', reason: 'r', recommendation: 'a' },
      { type: 'schedule-overrun', level: 'block', reason: 'r', recommendation: 'a' },
    ]
    const result = aggregateRisk(risks)
    expect(result.level).toBe('block')
  })
  ...
  it('多个 warn 风险仍返回 warn', () => {
    const risks: Risk[] = [
      { type: 'cost-overrun', level: 'warn', reason: 'r', recommendation: 'a' },
      { type: 'schedule-remaining', level: 'warn', reason: 'r', recommendation: 'a' },
    ]
    const result = aggregateRisk(risks)
    expect(result.level).toBe('warn')
  ...
  it('与 evaluateProject 联动返回正确总体等级', () => {
    const project = makeProject({ usedDays: 110, plannedDays: 100, cost: 110, budget: 100 })
    const risks = evaluateProject(project, makeWorkspace({ deliverables: [] }), makeConfig())
    const result = aggregateRisk(risks)
    expect(result.level).toBe('block')
    expect(result.items).toBe(risks)
