...
const fieldInput = (label: string) =>
  screen.getByText(label).parentElement!.querySelector('input,select,textarea') as HTMLInputElement

const uploadAndAnalyze = async () => {
  await userEvent.upload(
    screen.getByText('选择合同文件').querySelector('input')!,
...
describe('CreateProjectModal AI 合同分析模式', () => {
  ...
  it('默认进入 AI 模式且表单字段可见', () => {
    ...
    expect(screen.getByRole('tab', { name: 'AI 合同分析' }).getAttribute('aria-selected')).toBe('true')
    expect(screen.getByRole('tab', { name: '手动填写' }).getAttribute('aria-selected')).toBe('false')
  ...
  it('分析成功后回填各表单字段', async () => {
    ...
    await waitFor(() => expect(fieldInput('项目名称').value).toBe('AI 识别项目'))
    ...
    expect(
      (screen.getByText('备注').parentElement!.querySelector('textarea') as HTMLTextAreaElement).value,
    ).toBe('合同备注')
  ...
  it('分析失败展示服务端 detail 并可重试', async () => {
    ...
    vi.mocked(aiApi.analyzeProjectDraft).mockRejectedValueOnce(new ApiError('合同解析失败', 'PARSE_ERROR', 422))
    render(<CreateProjectModal onClose={() => {}} onCreated={() => {}} />)
    await uploadAndAnalyze()
  ...
  it('AI 回填后提交体结构与现状一致且不含 code', async () => {
    vi.mocked(aiApi.analyzeProjectDraft).mockResolvedValueOnce(draft)
    vi.mocked(projectsApi.createProject).mockResolvedValueOnce({ id: 1 } as never)
