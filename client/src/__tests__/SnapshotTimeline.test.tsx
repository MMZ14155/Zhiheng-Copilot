...
vi.mock('../api', async (original) => {
  ...
  return { ...actual, snapshotsApi: { listSnapshots: vi.fn(), getSnapshot: vi.fn(), restoreSnapshot: vi.fn() } };
});

const current = { hash: 'a'.repeat(64), parentHash: 'b'.repeat(64), author: '甲', message: '当前上传', createdAt: '2026-08-17T08:00:00Z', entryCount: 2 };
const older = { hash: 'b'.repeat(64), parentHash: null, author: '乙', message: '早期版本', createdAt: '2026-08-16T08:00:00Z', entryCount: 1 };

afterEach(() => { cleanup(); vi.clearAllMocks(); vi.unstubAllGlobals(); });

describe('SnapshotTimeline', () => {
  it('渲染时间线并仅在展开时加载文件树', async () => {
    vi.mocked(snapshotsApi.listSnapshots).mockResolvedValue({ projectId: 7, snapshots: [current, older] });
    vi.mocked(snapshotsApi.getSnapshot).mockResolvedValue({ ...older, projectId: 7, entries: [{ fileId: 3, path: '合同.pdf', version: 'c'.repeat(64), uploader: '丙', uploadedAt: '2026-08-16T07:00:00Z' }] });
  ...
  it('确认恢复后展示结果与 skipped 清单并刷新数据', async () => {
    ...
    vi.mocked(snapshotsApi.listSnapshots).mockResolvedValue({ projectId: 7, snapshots: [current, older] });
    vi.mocked(snapshotsApi.restoreSnapshot).mockResolvedValue({ snapshot: 'd'.repeat(64), restoredFiles: 1, skipped: [{ fileId: 8, path: '缺失.pdf', reason: '源版本不存在' }] });
    ...
    await waitFor(() => expect(snapshotsApi.listSnapshots).toHaveBeenCalledTimes(2));
  ...
  it('取消确认不请求恢复并原样展示 403 detail', async () => {
    ...
    vi.mocked(snapshotsApi.restoreSnapshot).mockRejectedValue(new ApiError('仅项目经理可恢复快照', 'FORBIDDEN', 403));
    await userEvent.click(screen.getByRole('button', { name: '恢复到此快照' }));
    await screen.findByText('仅项目经理可恢复快照');
  });

  it('处理加载错误、重试与空态', async () => {
    vi.mocked(snapshotsApi.listSnapshots).mockRejectedValueOnce(new Error()).mockResolvedValueOnce({ projectId: 7, snapshots: [] });
    render(<SnapshotTimeline projectId={7} onChanged={vi.fn()} />);
