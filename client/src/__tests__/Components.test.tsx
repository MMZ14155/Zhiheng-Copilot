...
vi.mock("../api", async (original) => {
  ...
  return {
    ...
    deliverablesApi: { ...actual.deliverablesApi, listTrackedFiles: vi.fn() },
    filesApi: {
      ...actual.filesApi,
    ...
    tagsApi: {
      ...
      listTags: vi.fn(),
      createTag: vi.fn(),
      listTagSnapshots: vi.fn(),
      createTagSnapshot: vi.fn(),
    },
  };
});
afterEach(() => {
...
describe("核心组件", () => {
  it("Header 展示默认与自定义副标题并退出", async () => {
    setAuthToken("t", { id: 1, login: "u", name: "U", isAdmin: false });
    const { rerender } = route(<Header />);
    expect(screen.getByText(/项目风险与经营分析助手/)).toBeTruthy();
  ...
  it("ChatArea 加载欢迎答复并发送输入与快捷问题", async () => {
    vi.mocked(aiApi.askCopilot)
      .mockResolvedValueOnce({ answer: "欢迎回答", references: ["合同"] })
      .mockResolvedValueOnce({ answer: "第二答复", references: [] })
      .mockResolvedValueOnce({ answer: "快捷答复", references: [] });
    route(<ChatArea projectId={3} />);
    expect(screen.getByText("思考中…")).toBeTruthy();
    await screen.findByText(/欢迎回答/);
    ...
    await screen.findByText(/第二答复/);
    await userEvent.click(screen.getByText("本周关键节点"));
    await screen.findByText(/快捷答复/);
  });
  ...
  it("ChatArea 初始失败显示兜底，发送失败显示 ApiError", async () => {
    ...
    vi.mocked(aiApi.askCopilot)
      ...
      .mockRejectedValueOnce(new ApiError("接口失败", "X", 500));
    route(<ChatArea />);
    await screen.findByText(/你好，我是/);
    const input = screen.getByPlaceholderText("输入问题...");
    await userEvent.type(input, "问题");
    fireEvent.keyDown(input, { key: "Enter" });
    await screen.findByText(/接口失败/);
  });

  it("VersionHistory 展示空状态和最新版本信息", async () => {
    const { rerender } = render(
      <VersionHistory projectId={1} deliverables={[]} />,
    );
  ...
  it("VersionHistory 处理加载错误和重试", async () => {
    vi.mocked(deliverablesApi.listTrackedFiles).mockRejectedValueOnce(
      new Error(),
    );
    render(
      <VersionHistory
    ...
    vi.mocked(deliverablesApi.listTrackedFiles).mockResolvedValueOnce([]);
    await userEvent.click(screen.getByText("重试"));
    await screen.findByText("暂无版本");
  });
  ...
  it("VersionHistory 通过 blob 下载最新版本并处理失败", async () => {
    ...
    vi.mocked(filesApi.downloadVersion).mockResolvedValueOnce({
      blob: new Blob(["x"]),
      filename: "report.pdf",
    });
    ...
    await waitFor(() =>
      expect(filesApi.downloadVersion).toHaveBeenCalledWith("abcdefghijk"),
    );
    expect(createObjectURL).toHaveBeenCalled();
    expect(revokeObjectURL).toHaveBeenCalledWith("blob:mock");
    createObjectURL.mockRestore();
    revokeObjectURL.mockRestore();
    vi.mocked(filesApi.downloadVersion).mockRejectedValueOnce(
      new Error("下载失败"),
    );
    await userEvent.click(screen.getByText("下载"));
  ...
  it("TagPanel 加载空状态、校验并创建标签", async () => {
    setAuthToken("t", { id: 1, login: "u", name: "测试员", isAdmin: false });
    vi.mocked(tagsApi.listTags).mockResolvedValue([]);
    vi.mocked(filesApi.listProjectFiles).mockResolvedValue([]);
    vi.mocked(tagsApi.createTag).mockResolvedValue({} as never);
    render(<TagPanel projectId={2} />);
    await screen.findByText("暂无标签");
    expect(screen.getByDisplayValue("测试员")).toBeTruthy();
    await userEvent.click(screen.getByText("新建标签"));
    expect(screen.getByRole("alert").textContent).toContain("请输入标签名称");
    await userEvent.type(screen.getByLabelText("标签名称"), " 标签 ");
    await userEvent.type(screen.getByPlaceholderText("选填"), "备注");
    await userEvent.click(screen.getByText("新建标签"));
    await waitFor(() =>
      expect(tagsApi.createTag).toHaveBeenCalledWith(2, {
        name: "标签",
        type: "demo",
        note: "备注",
      }),
  ...
  it("TagPanel 展开标签并创建快照", async () => {
    ...
    vi.mocked(tagsApi.listTagSnapshots).mockResolvedValue([]);
    vi.mocked(filesApi.listFileVersionOptions).mockResolvedValue([
      "version123",
    ]);
    vi.mocked(tagsApi.createTagSnapshot).mockResolvedValue({} as never);
    render(<TagPanel projectId={2} />);
    await screen.findByText("里程碑");
    await userEvent.click(screen.getByText("展开"));
    await screen.findByText("暂无快照");
    await userEvent.selectOptions(screen.getByLabelText("选择项目文件"), "3");
    await waitFor(() =>
      expect(
        (screen.getByLabelText("选择文件版本") as HTMLSelectElement).value,
      ).toBe("version123"),
    );
    await userEvent.click(screen.getByText("添加快照"));
    await waitFor(() => expect(tagsApi.createTagSnapshot).toHaveBeenCalled());
  });
});
