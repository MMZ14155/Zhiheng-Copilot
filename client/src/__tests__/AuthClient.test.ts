...
describe("api 令牌钩子与 authApi", () => {
  ...
  it("设置令牌后请求自动携带 Authorization Bearer 头", async () => {
    ...
    fetchMock.mockResolvedValue(
      jsonResponse(200, { page: 1, size: 20, total: 0, items: [] }),
    );
  ...
  it("401 响应清空内存令牌与当前用户", async () => {
    ...
    fetchMock.mockResolvedValue(
      jsonResponse(401, { detail: "未认证或令牌已失效", code: "UNAUTHORIZED" }),
    );
  ...
  it("登录失败时抛出服务端 detail 作为错误信息", async () => {
    fetchMock.mockResolvedValue(
      jsonResponse(401, { detail: "用户名或密码错误", code: "UNAUTHORIZED" }),
    );
    await expect(authApi.login("demo", "wrong")).rejects.toMatchObject({
      message: "用户名或密码错误",
      code: "UNAUTHORIZED",
      status: 401,
    });
  ...
  it("修改密码 401 原样透传 detail 并保留会话以便界面展示", async () => {
    ...
    fetchMock.mockResolvedValue(
      jsonResponse(401, { detail: "原密码错误", code: "UNAUTHORIZED" }),
    );
    await expect(
      authApi.changePassword("wrong-old", "new-password"),
    ).rejects.toMatchObject({ message: "原密码错误", status: 401 });
