...
const FALLBACK_WELCOME = '你好，我是智衡Copilot。你可以向我询问项目风险、关键节点和阻塞原因。';
...
export default function ChatArea({ projectId }: ChatAreaProps) {
  ...
  useEffect(() => {
    ...
    void aiApi.askCopilot(DEFAULT_QUESTION, projectId)
  ...
  const sendMessage = async (text: string) => {
    ...
    try {
      ...
      setMessages((prev) => [...prev, { role: 'bot', content: answer, references }]);
    } catch (error) {
    ...
    <div className="chat-area">
      ...
      <div className="quick-q">
        {QUICK_QUESTIONS.map((question) => (
          <button key={question} disabled={isLoading} onClick={() => void sendMessage(question)}>
            {question}
          </button>
        ))}
      ...
      <div className="chat-input">
        <input
          ...
          disabled={isLoading}
        />
        <button disabled={isLoading || !input.trim()} onClick={() => void sendMessage(input)}>
          发送
        </button>
      </div>
    </div>
