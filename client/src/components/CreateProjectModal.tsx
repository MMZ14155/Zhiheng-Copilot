...
const MISSING_FIELD_LABELS: Record<string, string> = {
  name:'项目名称', customer_name:'客户名称', parties:'签约方', contract_amount:'合同金额',
  signed_date:'签约日期', started_date:'启动日期', planned_delivery_date:'计划交付日期',
  project_type:'项目类型', notes:'备注',
};
...
export default function CreateProjectModal({ onClose, onCreated }: { onClose:()=>void; onCreated:()=>void|Promise<void> }) {
  ...
  const applyDraft = (draft: ProjectDraft) => {
    setValues(v => ({
      ...
      contractAmount: draft.contractAmount === null ? '' : String(draft.contractAmount),
      signedDate: draft.signedDate ?? '',
      startedDate: draft.startedDate ?? '',
      deliveryDate: draft.plannedDeliveryDate ?? '',
      notes: draft.notes ?? '',
    }));
    setParties(draft.parties.map(p => ({ role:p.role, name:p.name, contact:p.contact })));
  ...
  const analyze = async () => {
    ...
    try {
      ...
      setAnalysisError(reason instanceof ApiError ? reason.message : '合同分析失败，请稍后重试');
  ...
  return <div className="create-project-backdrop" onMouseDown={e => { if (e.target === e.currentTarget && !busy) onClose(); }}>
    <section className="create-project-modal" role="dialog" aria-modal="true" aria-labelledby={titleId}>
      <header><div><h2 id={titleId}>新建项目</h2><p>填写项目基本信息，带 * 的字段为必填项。</p></div><button type="button" aria-label="关闭新建项目弹窗" onClick={onClose} disabled={busy}>×</button></header>
      <div className="mode-switch" role="tablist" aria-label="创建模式">
        <button type="button" role="tab" aria-selected={mode==='ai'} className={mode==='ai'?'active':''} onClick={()=>setMode('ai')}>AI 合同分析</button>
        <button type="button" role="tab" aria-selected={mode==='manual'} className={mode==='manual'?'active':''} onClick={()=>setMode('manual')}>手动填写</button>
      </div>
      <form onSubmit={e => void submit(e)} noValidate>
        {apiError && <div className="form-submit-error" role="alert">{apiError}</div>}
        {mode==='ai' && <div className="ai-upload-panel">
          <label className="ai-file-picker">
            选择合同文件
            <input type="file" accept={CONTRACT_ACCEPT} disabled={analyzing} onChange={e=>{setContractFile(e.target.files?.[0] ?? null); setAnalysisError(null);}}/>
          </label>
          {contractFile && <span className="ai-file-name">{contractFile.name}</span>}
          <button type="button" className="primary-button" disabled={!contractFile || analyzing} onClick={()=>void analyze()}>{analyzing?'分析中…':'开始分析'}</button>
          <p className="ai-hint">上传合同文件后，AI 将自动识别项目信息并回填下方表单，可继续手动修改。</p>
          {analyzing && <div className="ai-skeleton"><div className="skeleton-line short"/><div className="skeleton-line"/><div className="skeleton-line"/></div>}
          {analysisError && !analyzing && <div className="ai-analysis-error"><span>{analysisError}</span><button type="button" className="secondary-button" onClick={()=>void analyze()}>重试</button></div>}
          {analyzed && !analyzing && !analysisError && missingFields.length>0 && <div className="ai-missing-banner">以下字段未能从合同识别，请补充：{missingFields.map(f => MISSING_FIELD_LABELS[f] ?? f).join('、')}</div>}
          {analyzed && !analyzing && !analysisError && !missingFields.length && <div className="ai-success-note">已根据合同内容回填表单，请核对后提交。</div>}
        </div>}
