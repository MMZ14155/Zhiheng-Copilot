...
export function evaluateProject(
  ...
  // 规则1 - 时间进度（强制检测）
  const scheduleRatio = project.plannedDays > 0 ? project.usedDays / project.plannedDays : 0;
 ...
 */
export function aggregateRisk(risks: Risk[]): { level: RiskLevel; items: Risk[] } {
...
assert(risks2.some((r) => r.type === 'schedule-overrun' && r.level === 'block'), '应存在进度超期阻塞');
assert(risks2.some((r) => r.type === 'cost-overrun' && r.level === 'block'), '应存在成本超支阻塞');
assert(risks2.some((r) => r.type === 'document-missing' && r.level === 'block'), '应存在资料缺失阻塞');
assert(risks2.some((r) => r.type === 'rule-conflict' && r.level === 'block'), '验收前无验收材料应触发规则冲突阻塞');
...
const conflictFile = makeTrackedFile(
  'F002',
  '交付成果',
  'ok',
  true,
  [makeVersion(false), makeVersion(false)]
);
...
assert(risks3.some((r) => r.type === 'schedule-remaining' && r.level === 'warn'), '剩余工期不足90天应触发预警');
assert(risks3.some((r) => r.type === 'version-conflict' && r.level === 'warn'), '多未冻结版本应触发版本冲突预警');
