...
export interface CopilotAskRequestDto {
  question: string;
  project_id?: number;
}
...
export interface RelatedProjectSummaryDto {
  ...
  signed_date: string | null;
  link_id: number;
  link_type: ProjectLinkTypeDto;
}
export interface DeliverableSummaryDto {
  id: number;
  name: string;
  created_at: string;
  updated_at: string;
...
export interface SummaryInputDto {
  ...
  tracked_file_name: string | null;
  file_version: string;
}
export interface LatestSummaryDto {
  id: number;
  version_no: number;
  content: string | null;
  created_by: string | null;
  created_at: string;
  inputs: SummaryInputDto[];
}
...
export interface ProjectLinkCreateDto {
  target_project_id: number;
  link_type: ProjectLinkTypeDto;
  note?: string | null;
}
export interface ProjectLinkResponseDto {
  id: number;
...
export interface RenewalChainResponseDto {
  project_id: number;
  depth_limit: number;
  items: ProjectResponseDto[];
}
...
export interface ProjectMemberAssignDto {
  user_id: number;
  role: ProjectMemberRoleDto;
}
...
export interface LlmConfigTestResponseDto {
  ok: boolean;
  detail: string;
}
...
export interface AverageMetricDto {
  value: number | null;
  sample_count: number;
}
export interface StageStatisticsDto {
  stage: ProjectStageDto | null;
  count: number;
...
export interface PaymentStatisticsDto {
  ...
  data_incomplete_projects: number;
}
...
export interface ProjectFileLatestVersionDto {
  version: string;
...
export interface ProjectFileResponseDto {
  ...
  created_at: string;
  updated_at: string;
  latest_version: ProjectFileLatestVersionDto | null;
}
export interface ProjectFileListResponseDto {
...
export interface TrackedFileListResponseDto {
  items: TrackedFileResponseDto[];
}
...
export interface TagSnapshotCreateDto {
  source_file_id: number;
  version: string;
  note?: string | null;
}
export interface TagSnapshotResponseDto {
  id: number;
  tag_id: number;
  source_file_id: number | null;
  file_version: string;
...
export interface SnapshotTimelineResponseDto {
  project_id: number;
...
export interface SnapshotDetailResponseDto extends SnapshotSummaryDto {
  project_id: number;
  entries: SnapshotEntryDto[];
}
export interface SkippedSnapshotFileDto {
  file_id: number;
  path: string;
  reason: string;
}
export interface SnapshotRestoreResponseDto {
  snapshot: string;
  restored_files: number;
  skipped: SkippedSnapshotFileDto[];
}
export interface WorkspaceAddOperationDto {
  op: "add";
  name: string;
  content: string;
...
export interface WorkspaceUpdateOperationDto {
  op: "update";
  file_id: number;
  content: string;
  changelog?: string | null;
}
export interface WorkspaceRemoveOperationDto {
  op: "remove";
  file_id: number;
}
export interface WorkspaceCommitRequestDto {
  ...
  operations: Array<
    ...
    | WorkspaceRemoveOperationDto
  >;
}
export interface WorkspaceCommitResponseDto {
  snapshot: string;
  message: string;
}
export interface TaskCreatedResponseDto {
  task_id: number;
  status: "pending";
}
export interface SummaryAnswerDto {
  question: string;
  answer: string;
}
export interface SummaryAnswersRequestDto {
  answers: SummaryAnswerDto[];
}
export interface SummaryAnswersTaskResponseDto {
  task_id: number;
  accepted_questions: string[];
  ignored_questions: string[];
}
...
export interface SummaryResponseDto {
  ...
  project_id: number;
  version_no: number;
  core_info: JsonObject;
  contract_invoice_progress: JsonObject;
  missing_materials: Array<Record<string, string>>;
...
export interface SummaryHistoryResponseDto {
  items: SummaryResponseDto[];
}
export interface ContractInfoResponseDto {
  type: "contract";
  id: number;
  version: string;
  ...
  amount: number | null;
  signed_date: string | null;
  payment_terms: Array<Record<string, string>>;
  missing_fields: string[];
  raw_output: JsonObject;
  created_at: string;
}
export interface InvoiceInfoResponseDto {
  type: "invoice";
  id: number;
  version: string;
