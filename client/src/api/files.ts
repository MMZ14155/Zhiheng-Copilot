...
export interface CreateFileUpload extends FileUpload {
  name: string;
  docType?: string;
}
...
const fileToBase64 = (file: File): Promise<string> =>
  new Promise((resolve, reject) => {
    ...
    reader.onerror = (event) =>
      reject(new Error(event?.target?.error?.message ?? "文件读取失败"));
...
export function createFile(
  ...
  return multipartRequest(
    `/projects/${projectId}/files${queryString({ name: input.name, changelog: input.changelog, doc_type: input.docType })}`,
    data,
  );
}
export function appendFileVersion(
  ...
  data.append("file", input.file);
  return multipartRequest(
    `/files/${fileId}/versions${queryString({ changelog: input.changelog })}`,
    data,
  );
}
export const listFileVersions = (id: number) =>
  jsonRequest<VersionListResponseDto>(`/files/${id}/versions`);
export const listFileVersionOptions = async (id: number): Promise<string[]> =>
  (await listFileVersions(id)).versions.map((item) => item.version);
...
export async function workspaceCommit(
...
): Promise<WorkspaceCommitResponseDto> {
  const operations: Array<
    ...
    | { op: "update"; file_id: number; content: string; changelog?: string }
    | { op: "remove"; file_id: number }
  > = [];
  for (const op of input.operations) {
    if (op.op === "add") {
      operations.push({
        op: "add",
        name: op.name,
        content: await fileToBase64(op.file),
        doc_type: op.docType,
